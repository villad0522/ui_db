import {
  ...
  getColumnIdFromName_core,  // カラム名からIDを取得
} from "./110_column_name.js";
...
//#######################################################################################
// 関数「getColumnIdFromName_core」に、引数と戻り値のチェック機能を追加した関数
//
export async function getColumnIdFromName( columnName ){
  //--------------------------------------------------------------------------
  // 引数を検証
  if( typeof columnName !== "string" ){
    if( !columnName ){
      throw new Error(`columnNameがNULLです。\nレイヤー : column_name\n関数 : getColumnIdFromName`);
    }
    else{
      throw new Error(`columnNameが文字列ではありません。\nレイヤー : column_name\n関数 : getColumnIdFromName`);
    }
  ...
  let result;
  try{
    result = await getColumnIdFromName_core( columnName );
  }
  catch(error){
    if( typeof error === "string" ){
      throw new Error(`${error}\nレイヤー : column_name\n関数 : getColumnIdFromName`);
