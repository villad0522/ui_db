...
export async function runApi_core( httpMethod, endpointPath, queryParameters, requestBody, isRequestFormData, isResponseFormData ){
  ...
  switch(apiInfo.commandName){
    ...
    //======================================================================
    case "ADD_COLUMN_PATH":{
      if(bugMode === 3) throw "MUTATION3";  // 意図的にバグを混入させる（ミューテーション解析）
      const viewId = Number(queryParameters["view"]);
      return await addColumnPath(viewId,requestBody);
    }
    //======================================================================
    case "AUTO_CORRECT_COLUMN_PATH":{
      if(bugMode === 4) throw "MUTATION4";  // 意図的にバグを混入させる（ミューテーション解析）
      const viewId = Number(queryParameters["view"]);
      return {
          "newColumnPathInput_option": await autoCorrectColumnPath(viewId,requestBody),
      };
    }
    //======================================================================
    case "CREATE_VIEW_COLUMN":{
    ...
    case "DELETE_CONDITION":{
      ...
      const viewId = queryParameters["view"];
      const conditionId = queryParameters["condition"];
      const newQueryParameters = await deleteCondition( viewId, conditionId, queryParameters );
      return {
    ...
    case "ADD_CONDITION":{
      ...
      for( const { viewId, newExtractionTarget: viewColumnName, newExtractionExpression, newExtractionValue } of requestBody.views ){
        if(bugMode === 8) throw "MUTATION8";  // 意図的にバグを混入させる（ミューテーション解析）
        await addCondition( viewColumnName, newExtractionExpression, newExtractionValue );
      }
      return {
        "nextUrl": "./?" + await convertQuery_core(queryParameters),
      }
    ...
    //======================================================================
    case "AUTO_CORRECT_CONDITION":{
      if(bugMode === 9) throw "MUTATION9";  // 意図的にバグを混入させる（ミューテーション解析）
      const views = [];
      for( let i=0; i<requestBody.views.length; i++ ){
        if(bugMode === 10) throw "MUTATION10";  // 意図的にバグを混入させる（ミューテーション解析）
        const { viewId, newExtractionTarget: viewColumnName, newExtractionValue } = requestBody.views[i];
        views[i] = {
          "newExtractionValue_option": await autoCorrectConditionalValue( viewColumnName, newExtractionValue ),
        };
      }
      return {
        "views": views,
    ...
    case "SCAN_KEYWORDS":{
      ...
      scanKeywords( tableId );
      return {
        "nextUrl": `/default/scan_keywords_progress/index.html?table=${tableId}`,
      };
    }
    //======================================================================
    case "GET_SCAN_KEYWORDS_PROGRESS":{
      if(bugMode === 24) throw "MUTATION24";  // 意図的にバグを混入させる（ミューテーション解析）
      return await getScanKeywordsProgress();
    }
    //======================================================================
    case "STOP_SCAN_KEYWORDS":{
      if(bugMode === 25) throw "MUTATION25";  // 意図的にバグを混入させる（ミューテーション解析）
      return {
        "userMessage": await stopScanKeywords(),
      };
    }
    //======================================================================
    case "TRANSFER_DATA":{
