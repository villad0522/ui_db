...
export async function runApi_core( httpMethod, endpointPath, queryParameters, requestBody, isRequestFormData, isResponseFormData ){
  ...
  switch(apiInfo.commandName){
    ...
    case "DELETE_CONDITION":{
      ...
      const newQueryParameters = await deleteCondition( conditionId, queryParameters );
      return {
    ...
    case "ADD_CONDITION":{
      ...
      const viewId = apiInfo.viewId;
      const viewColumnName = requestBody[`newExtractionTarget${viewId}`];
      const conditionalExpression = requestBody[`newExtractionExpression${viewId}`];
      const conditionalValue = requestBody[`newExtractionValue${viewId}`];
      await addCondition( viewColumnName, conditionalExpression, conditionalValue );
      return {
        "nextUrl": "./?" + await convertQuery_core(queryParameters),
      }
    }
    //======================================================================
    case "AUTO_CORRECT_CONDITIONAL_VALUE":{
      if(bugMode === 5) throw "MUTATION5";  // 意図的にバグを混入させる（ミューテーション解析）
      const viewId = apiInfo.viewId;
      const viewColumnName = requestBody[`newExtractionTarget${viewId}`];
      const inputText = requestBody[`newExtractionValue${viewId}`];
      return {
        [`newExtractionTarget${viewId}_option`]: await autoCorrectConditionalValue( viewColumnName, inputText ),
      }
    }
    //======================================================================
