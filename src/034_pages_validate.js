...
//
export async function createPage( parentPageId, pageName, isInDynamicList ){
  //--------------------------------------------------------------------------
  // 引数を検証
  ...
  if( typeof isInDynamicList !== "boolean" ){
    if( !isInDynamicList ){
      throw new Error(`isInDynamicListがNULLです。\nレイヤー : pages\n関数 : createPage`);
    }
    else{
      throw new Error(`isInDynamicListがブール値ではありません。\nレイヤー : pages\n関数 : createPage`);
    }
  }
  else if( isNaN(isInDynamicList) ){
    throw new Error(`isInDynamicListがブール値ではありません。\nレイヤー : pages\n関数 : createPage`);
  }
  //
  //--------------------------------------------------------------------------
  ...
  let result;
  try{
    result = await createPage_core( parentPageId, pageName, isInDynamicList );
  }
  catch(error){
    if( typeof error === "string" ){
      throw new Error(`${error}\nレイヤー : pages\n関数 : createPage`);
...
export async function getPageInfo( pageId ){
  ...
  if( typeof result.dynamicListType !== "string" ){
    if( !result.dynamicListType ){
      throw new Error(`result.dynamicListTypeがNULLです。\nレイヤー : pages\n関数 : getPageInfo`);
    }
    else{
      throw new Error(`result.dynamicListTypeが文字列ではありません。\nレイヤー : pages\n関数 : getPageInfo`);
    }
  }
  //
  //--------------------------------------------------------------------------
