...
export async function buyData_core(  ){
  ...
  await createColumn( t1, "請求日", "INTEGER", null );
  ...
  await createColumn( t1, "入荷年度", "INTEGER", null );
  await createColumn( t1, "入荷月", "INTEGER", null );
  ...
  await runSqlWriteOnly(
    `INSERT INTO 購入データ (
        ...
        updated_at
          ...
          CAST( STRFTIME('%d', 列3 / 1000, 'UNIXEPOCH', 'LOCALTIME' ) AS INTEGER ) AS '請求日',
          CAST( STRFTIME( '%Y', 列4 / 1000, 'UNIXEPOCH', 'LOCALTIME', '-3 MONTHS', 'START OF YEAR' ) AS INTEGER ) AS '入荷年度',
          CAST( STRFTIME('%m', 列4 / 1000, 'UNIXEPOCH', 'LOCALTIME' ) AS INTEGER ) AS '入荷月',
          CAST( STRFTIME('%d', 列4 / 1000, 'UNIXEPOCH', 'LOCALTIME' ) AS INTEGER ) AS '入荷日',
          CAST( 列5 AS INTEGER ),
...
export async function broodbookData_core(  ){
  ...
  await createColumn( t1, "退舎予定年度", "INTEGER", null );
  await createColumn( t1, "退舎予定月", "INTEGER", null );
  await createColumn( t1, "退舎予定日", "INTEGER", null );
  await createColumn( t1, "出生年度", "INTEGER", null );
  await createColumn( t1, "出生月", "INTEGER", null );
  ...
  await runSqlWriteOnly(
    `INSERT INTO 飼育台帳データ (
        ...
        updated_at
          ...
          CAST( STRFTIME( '%d', 列14 / 1000, 'UNIXEPOCH', 'LOCALTIME' ) AS INTEGER ),
          ...
          CAST( STRFTIME( '%d', 列15 / 1000, 'UNIXEPOCH', 'LOCALTIME' ) AS INTEGER ),
          列16,
          CAST( STRFTIME( '%Y', 列17 / 1000, 'UNIXEPOCH', 'LOCALTIME', '-3 MONTHS', 'START OF YEAR' ) AS INTEGER ),
          CAST( STRFTIME( '%m', 列17 / 1000, 'UNIXEPOCH', 'LOCALTIME' ) AS INTEGER ),
          CAST( STRFTIME( '%d', 列17 / 1000, 'UNIXEPOCH', 'LOCALTIME' ) AS INTEGER ),
          CAST( STRFTIME( '%Y', 列18 / 1000, 'UNIXEPOCH', 'LOCALTIME', '-3 MONTHS', 'START OF YEAR' ) AS INTEGER ),
          CAST( STRFTIME( '%m', 列18 / 1000, 'UNIXEPOCH', 'LOCALTIME' ) AS INTEGER ),
          CAST( STRFTIME( '%d', 列18 / 1000, 'UNIXEPOCH', 'LOCALTIME' ) AS INTEGER ),
