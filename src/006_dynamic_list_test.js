...
import {
  startUp,  // プログラム起動
  createDynamicList,  // 動的リストを作成
  deleteDynamicList,  // 動的リストを削除
  generateSQL,  // SQLクエリを生成
} from "./007_dynamic_list_validate.js";
import { setBugMode } from "./008_dynamic_list.js";
...
export async function test006() {
    ...
    for ( i = 1; i <= 7; i++ ) {
        ...
        console.log(`レイヤー「dynamic_list」からバグは見つかりませんでしたが、テストコードが不十分です。意図的に発生させたバグ(bugMode: ${ i })を検出できませんでした。\n\n`);
        return;
    }
    // 意図的に埋め込んだ全てのバグを、正常に検出できた
    setBugMode(0);    // 意図的なバグの発生を止める
    console.log(`レイヤー「dynamic_list」からバグは見つかりませんでした。また、意図的に${ i-1 }件のバグを発生させたところ、全てのバグを検知できました。\n\n`);
    return;
}
