...
export async function startUp_core( localUrl, isDebug ){
  ...
  await runSqlWriteOnly(
    `CREATE TABLE IF NOT EXISTS pages (
      ...
      FOREIGN KEY (dynamic_parent_id) REFERENCES joined_tables(page_id)
    );`, {},
  );
...
export async function getPageInfo_core( pageId ){
  ...
  const pages = await runSqlReadOnly(
    `SELECT 
        ...
        page1.memo AS memo,
        joined_tables.table_id AS tableId,
        joined_tables.joined_table_type AS joinedTableType,
      ...
      FROM pages AS page1
        LEFT OUTER JOIN joined_tables
          ON joined_tables.page_id = page1.page_id
