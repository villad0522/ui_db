  ...
  for( const { joinedColumnId, joinedColumnType, joinId, columnName, joinedColumnName } of selectData ){
    ...
    switch(joinedColumnType){
      case "RAW":
        ...
        selectList.push(`${columnName} AS "${joinedColumnName}"`);
        break;
      case "SUM":
        if(bugMode === 4) throw "MUTATION4";  // 意図的にバグを混入させる（ミューテーション解析）
        selectList.push(`SUM(${columnName}) AS "${joinedColumnName}"`);
        break;
      case "MAX":
        if(bugMode === 5) throw "MUTATION5";  // 意図的にバグを混入させる（ミューテーション解析）
        selectList.push(`MAX(${columnName}) AS "${joinedColumnName}"`);
        break;
      case "MIN":
        if(bugMode === 6) throw "MUTATION6";  // 意図的にバグを混入させる（ミューテーション解析）
        selectList.push(`MIN(${columnName}) AS "${joinedColumnName}"`);
        break;
      case "AVG":
        if(bugMode === 7) throw "MUTATION7";  // 意図的にバグを混入させる（ミューテーション解析）
        selectList.push(`AVG(${columnName}) AS "${joinedColumnName}"`);
        break;
      case "COUNT":
        if(bugMode === 8) throw "MUTATION8";  // 意図的にバグを混入させる（ミューテーション解析）
        selectList.push(`COUNT(${columnName}) AS "${joinedColumnName}"`);
        break;
      default:
  ...
  for( const { joinedColumnId, conditionalExpression, joinId, columnName, conditionValue } of whereData ){
    ...
    switch(conditionalExpression.trim()){
      case "=":
        ...
        whereList.push(`( ${columnName} = :${joinedColumnId} )`);
        break;
      case "!=":
