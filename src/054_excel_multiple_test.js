...
import {
  getExcelId,  // 表示すべきExcelのIDを取得する
} from "./055_excel_multiple_validate.js";
import { setBugMode } from "./056_excel_multiple.js";
...
export async function test054() {
    ...
    for ( i = 1; i <= 4; i++ ) {
        ...
        console.log(`レイヤー「excel_multiple」からバグは見つかりませんでしたが、テストコードが不十分です。意図的に発生させたバグ(bugMode: ${ i })を検出できませんでした。\n\n`);
        return;
    }
    // 意図的に埋め込んだ全てのバグを、正常に検出できた
    setBugMode(0);    // 意図的なバグの発生を止める
    console.log(`レイヤー「excel_multiple」からバグは見つかりませんでした。また、意図的に${ i-1 }件のバグを発生させたところ、全てのバグを検知できました。\n\n`);
    return;
}
