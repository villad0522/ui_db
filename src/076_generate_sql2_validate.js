...
} from "./077_generate_sql2.js";


//#######################################################################################
// 関数「generateSQLwithoutDuplication_core」に、引数と戻り値のチェック機能を追加した関数
//
export async function generateSQLwithoutDuplication( tableId, selectData, joinData, whereData, orderData, isCount, onePageMaxSize ){
  //--------------------------------------------------------------------------
  // 引数を検証
  ...
  for( let i=0; i<whereData.length; i++ ){
    ...
    if( typeof whereData[i].conditionId !== "string" ){
      if( !whereData[i].conditionId ){
        throw new Error(`whereData[${i}].conditionIdがNULLです。\nレイヤー : generate_sql2\n関数 : generateSQLwithoutDuplication`);
      }
      else{
        throw new Error(`whereData[${i}].conditionIdが文字列ではありません。\nレイヤー : generate_sql2\n関数 : generateSQLwithoutDuplication`);
      }
    }
  ...
  let result;
  try{
    result = await generateSQLwithoutDuplication_core( tableId, selectData, joinData, whereData, orderData, isCount, onePageMaxSize );
  }
  catch(error){
