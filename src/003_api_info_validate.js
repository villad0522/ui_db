...
export async function getEndpointInfo( endpointPath ){
  ...
  if( !Array.isArray(result.commands) ){
    if( !result.commands ){
      throw new Error(`result.commandsがNULLです。\nレイヤー : api_info\n関数 : getEndpointInfo`);
    }
    else{
      throw new Error(`result.commandsが配列ではありません。\nレイヤー : api_info\n関数 : getEndpointInfo`);
    }
  }
  for( let i=0; i<result.commands.length; i++ ){
    if( typeof result.commands[i] !== "object" ){
      if( !result.commands[i] ){
        throw new Error(`result.commands[${i}]がNULLです。\nレイヤー : api_info\n関数 : getEndpointInfo`);
      }
      else{
        throw new Error(`result.commands[${i}]がオブジェクトではありません。\nレイヤー : api_info\n関数 : getEndpointInfo`);
      }
    }
    if( typeof result.commands[i].resultNameSpace !== "string" ){
      if( !result.commands[i].resultNameSpace ){
        throw new Error(`result.commands[${i}].resultNameSpaceがNULLです。\nレイヤー : api_info\n関数 : getEndpointInfo`);
      }
      else{
        throw new Error(`result.commands[${i}].resultNameSpaceが文字列ではありません。\nレイヤー : api_info\n関数 : getEndpointInfo`);
      }
    }
    if( typeof result.commands[i].commandName !== "string" ){
      if( !result.commands[i].commandName ){
        throw new Error(`result.commands[${i}].commandNameがNULLです。\nレイヤー : api_info\n関数 : getEndpointInfo`);
      }
      else{
        throw new Error(`result.commands[${i}].commandNameが文字列ではありません。\nレイヤー : api_info\n関数 : getEndpointInfo`);
      }
    }
    if( result.commands[i].additionalParameters===null || result.commands[i].additionalParameters===undefined ){
      throw new Error(`result.commands[${i}].additionalParametersがNULLです。\nレイヤー : api_info\n関数 : getEndpointInfo`);
    }
    else if( typeof result.commands[i].additionalParameters !== "object" ){
      throw new Error(`result.commands[${i}].additionalParametersがオブジェクトではありません。\nレイヤー : api_info\n関数 : getEndpointInfo`);
    }
    else if( result.commands[i].additionalParameters.constructor !== Object ){
      throw new Error(`result.commands[${i}].additionalParametersが辞書型ではありません。\nレイヤー : api_info\n関数 : getEndpointInfo`);
    }
    for( const j in result.commands[i].additionalParameters ){
      if( typeof j !== "string" ){
        throw new Error(`result.commands[${i}].additionalParametersのキーが文字列ではありません。\nレイヤー : api_info\n関数 : getEndpointInfo`);
      }
    }
  }
  //
