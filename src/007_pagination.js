...
export async function runApi_core( httpMethod, endpointPath, queryParameters, requestBody, isRequestFormData, isResponseFormData ){
  if(bugMode === 1) throw "MUTATION1";  // 意図的にバグを混入させる（ミューテーション解析）
    if (!isResponseFormData) {
        ...
        return await runApi( endpointPath, queryParameters, requestBody );
    }
    //
    // エンドポイントの情報を取得する
    const endpointInfo = await getEndpointInfo( endpointPath );
    //
    // APIのメイン処理を実行する
    let response = await runApi( endpointPath, queryParameters, requestBody );
    //
...
export async function getEndpointInfo_core( endpointPath, isRequestFormData, isResponseFormData ){
  if(bugMode === 3) throw "MUTATION3";  // 意図的にバグを混入させる（ミューテーション解析）
    if (!isResponseFormData) {
        ...
        // （HTML側にすり合わせる必要がなければ）
        //  何もしない（下層の機能をそのまま上層に提供する）
        return await getEndpointInfo( endpointPath );
    }
    const endpointInfo1 = await getEndpointInfo( endpointPath );
