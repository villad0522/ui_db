...
export async function generateSQLwithDuplication( tableId, selectData, joinData, whereData, orderData ){
  ...
  for( let i=0; i<selectData.length; i++ ){
    ...
    if( typeof selectData[i].displayColumnId !== "string" ){
      if( !selectData[i].displayColumnId ){
        throw new Error(`selectData[${i}].displayColumnIdがNULLです。\nレイヤー : generate_sql1\n関数 : generateSQLwithDuplication`);
      }
      else{
        throw new Error(`selectData[${i}].displayColumnIdが文字列ではありません。\nレイヤー : generate_sql1\n関数 : generateSQLwithDuplication`);
      }
    }
