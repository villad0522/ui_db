...
export async function generateSQLwithoutDuplication( tableId, selectData, joinData, whereData, orderData ){
  ...
  for( let i=0; i<joinData.length; i++ ){
    ...
    if( typeof joinData[i].toColumnId !== "string" ){
      if( !joinData[i].toColumnId ){
        throw new Error(`joinData[${i}].toColumnIdがNULLです。\nレイヤー : generate_sql2\n関数 : generateSQLwithoutDuplication`);
      }
      else{
        throw new Error(`joinData[${i}].toColumnIdが文字列ではありません。\nレイヤー : generate_sql2\n関数 : generateSQLwithoutDuplication`);
      }
    }
  }
